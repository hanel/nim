...
nim = function(formula =   ~ 1, data, method = 'Nelder-Mead', tol = 0.1, try_shape = NA, force = FALSE, ...){

  inim = function(formula, data, method, tol, ini_shape = NA){
    ...
    if (!is.na(ini_shape)) ini$shape = ini_shape
   ...
   extremes(data) = ext
   out = structure(.Data = THETA, logLik = -nll.gev(par = THETA, fpar = lpar, dat = as.matrix(dmx)), call = match.call(), class = 'nim', data = data)#, resid = provideResid(data, THETA), observed = data, h = h, call = match.call(), class = 'nprgev')
 ...
 if ( (all(!is.na(try_shape)) & logLik(n)==-1e20) |  (all(!is.na(try_shape)) & force==TRUE)) {
   ...
   for (i in try_shape){
    message('>> Trying ', i, '\n')
    cl = call('inim', formula = formula, data = data, method = method, tol = tol, ini_shape = i)
    N[[length(N) + 1]] = eval(cl)
   }
